...
def init_model(args):
    ...
    print(f'MiniMind parameters: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)')
...
def main():
    ...
    parser.add_argument('--inference_rope_scaling', default=False, action='store_true', help="Enable RoPE positional encoding extrapolation (4x, only addresses positional encoding issues)")
    ...
    parser.add_argument('--historys', default=0, type=int, help="Number of conversation history turns to carry (must be even, 0 means no history)")
